...
def conv_forward_naive(x, w, b, conv_param):
  ...
  for i in range(H_out):
      for j in range(W_out):
          ...
          #out[:, : , i, j] = np.sum(x_pad_masked * w[:, :, :, :], axis=(1,2,3)) # 这个不行，因为广播发生在两个维度不同的数组之间
          
  #for k in range(F):
      #out[:, k, :, :] = out[:, k, :, :] + b[k]  #broadcast
  out = out + (b)[None, :, None, None] # more efficent
  #pass
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def conv_backward_naive(dout, cache):
  ...
  db = np.sum(dout, axis = (0,2,3))  # dout默认已经在上游/N了，至于2,3维就是被扩大了，不用考虑除
  ...
  for i in range(H_out):
      for j in range(W_out):
          ...
          for n in range(N): #compute dx_pad 
              # 每次从x_pad中取和W形状相同的子矩阵，并将该子矩阵原位加上W*dout后的元素。dout (N, F, HH, WW)，每一个F独立计算，对N那个维度要求和(batch平均)
              dx_pad[n, :, i*stride:i*stride+HH, j*stride:j*stride+WW] += np.sum((w[:, :, :, :] * 
                                                 (dout[n, :, i, j])[:,None ,None, None]), axis=0) # FIXME: did it rotted? A:yes
  dx = dx_pad[:,:,pad:-pad,pad:-pad] # -pad表示从右往左取值
  #pass
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def max_pool_backward_naive(dout, cache):
  ...
  for i in xrange(H_out):
     for j in xrange(W_out):
        ...
        dx[:,:,i*stride : i*stride+HH, j*stride : j*stride+WW] += temp_binary_mask * (dout[:,:,i,j])[:,:,None,None] # poolling区域没有重叠就不用+=
  #pass
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
