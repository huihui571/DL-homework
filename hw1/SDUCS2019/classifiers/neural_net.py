...
class ThreeLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    dscores /= N      #FIXME: minibatch内平均? 可是后面也dh,dW也没求和啊？ A：求和了，矩阵相乘展开，结果矩阵每一个元素都是求和啊
    # Third  Layer
    dh2 = np.dot(dscores, W3.T)       # (N, H)
    dW3 = np.dot(h2.T, dscores)       # (H, C)
    db3 = np.sum(dscores, axis=0)     # (C, )   #同一个偏置b被加到每一个样本数据上，反向传播时需汇总。因为上面对误差/N，因此最终实际相当于minibath内取平均
    #ReLU
    dh2[z2<=0] = 0
    dz2 = dh2         # (N, H)
    # Second Layer
